...
def call_cloudflare_ai(prompt: str) -> str:
    ...
    model = os.environ.get("CF_AI_MODEL", "@cf/qwen/qwen3-30b-a3b-fp8").strip()
...
def validate_brief(brief: dict) -> None:
    ...
    for item in items:
        ...
        if item.get("summary_zh") and float(item.get("source_confidence") or 0) < MIN_DIRECT_CONTEXT_CONFIDENCE:
